...
namespace SistemaGestaoBiblioteca.UnitTests.Commands.Livros
{
    public class AlterarCadatroLivrosTests
    {
        ...
        public async Task FalhaAoAtualizarLivroPoisNaoEncontrado()
        {
            ...
            mockUnitOfWork.Setup(u => u.LivroRepository.GetAsync(command.Livro.Id)).ReturnsAsync(null as Livro);
        ...
        public async Task FalhaAoAtualizarLivroPoisAutorNaoEncontrado()
        {
            ...
            mockUnitOfWork.Setup(u => u.LivroRepository.GetAsync(command.Livro.Id)).ReturnsAsync(livro);
            mockUnitOfWork.Setup(u => u.AutorRepository.GetAsync(command.Livro.Autor.Id)).ReturnsAsync(null as Autor);
        ...
        public async Task FalhaAoAtualizarLivroPoisGeneroNaoEncontrado()
        {
            ...
            mockUnitOfWork.Setup(u => u.LivroRepository.GetAsync(command.Livro.Id)).ReturnsAsync(livro);
            mockUnitOfWork.Setup(u => u.AutorRepository.GetAsync(command.Livro.Autor.Id)).ReturnsAsync(autor);
            mockUnitOfWork.Setup(u => u.GeneroRepository.GetAsync(command.Livro.Genero.Id)).ReturnsAsync(null as Genero);
        ...
        public async Task AlterarLivroComSucesso()
        {
            ...
            mockUnitOfWork.Setup(u => u.LivroRepository.GetAsync(command.Livro.Id)).ReturnsAsync(livro);
            mockUnitOfWork.Setup(u => u.AutorRepository.GetAsync(command.Livro.Autor.Id)).ReturnsAsync(autor);
            mockUnitOfWork.Setup(u => u.GeneroRepository.GetAsync(command.Livro.Genero.Id)).ReturnsAsync(genero);
